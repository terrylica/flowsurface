// NOTE(fork): Turnover imbalance — capital-weighted OFI, bounded [-1, 1].
...
/// Turnover imbalance indicator: capital-weighted buy/sell flow, [-1, 1].
/// Signed histogram centered on zero.
/// Only available for ODB bars with microstructure data from ClickHouse.
pub struct TurnoverImbalanceIndicator {
    cache: Caches,
    data: BTreeMap<u64, f32>,
}

impl TurnoverImbalanceIndicator {
    pub fn new() -> Self {
        Self {
    ...
    fn indicator_elem<'a>(
        ...
        if self.data.is_empty() {
            return center(text("Turnover Imbalance: no data")).into();
        }

        let tooltip = |value: &f32, _next: Option<&f32>| {
            let sign = if *value >= 0.0 { "+" } else { "" };
            PlotTooltip::new(format!("Turnover Imbalance: {sign}{:.4}", value))
        };
...
impl KlineIndicatorImpl for TurnoverImbalanceIndicator {
    ...
    fn rebuild_from_source(&mut self, source: &PlotData<KlineDataPoint>) {
        match source {
            ...
            PlotData::TickBased(tickseries) => {
                self.data = tickseries.turnover_imbalance_data();
            }
        }
            ...
            PlotData::TickBased(tickseries) => {
                ...
                for (idx, dp) in tickseries.datapoints.iter().enumerate().skip(start_idx) {
                    if let Some(m) = dp.microstructure
                        && let Some(ti) = m.turnover_imbalance
                    {
                        self.data.insert(idx as u64, ti);
                    }
                }
            }
        }
